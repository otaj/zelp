...
void main() {
  group('GpsDownloadPlan', () {
    test('rejects empty selection without UIHH', () {
      expect(
        () => GpsDownloadPlan(selected: {}, buildUihh: false),
        throwsArgumentError,
      );
    });

    test('UIHH-only fetches AGPSZIP+LLE privately, exports none of them', () {
      final plan = GpsDownloadPlan(selected: {}, buildUihh: true);
      expect(plan.fetchTypes, {GpsFileType.agpsZip, GpsFileType.lle});
    ...
    test('selected types are exported; UIHH adds private fetch only', () {
      final plan = GpsDownloadPlan(
        selected: {GpsFileType.epo},
        buildUihh: true,
      );
      expect(plan.fetchTypes, {
        GpsFileType.epo,
        GpsFileType.agpsZip,
        GpsFileType.lle,
      });
      expect(plan.exportTypes, {GpsFileType.epo});
    ...
    test('explicit AGPSZIP+LLE with UIHH exports those zips too', () {
      final plan = GpsDownloadPlan(
        selected: {GpsFileType.agpsZip, GpsFileType.lle},
        buildUihh: true,
      );
    ...
    test('without UIHH fetch and export are exactly the selection', () {
      final plan = GpsDownloadPlan(
        selected: {GpsFileType.epo, GpsFileType.lto},
        buildUihh: false,
      );
      expect(plan.fetchTypes, {GpsFileType.epo, GpsFileType.lto});
      expect(plan.exportTypes, {GpsFileType.epo, GpsFileType.lto});
  ...
  group('GpsFileType', () {
    test('apiOrder covers every enum value exactly once', () {
      ...
      expect(
        GpsFileType.values.map((t) => t.apiName).toSet().length,
  ...
  group('findNamedGpsPayload', () {
    test('returns first matching name substring', () {
      final files = [
        (fileName: 'other.zip', bytes: Uint8List.fromList([1])),
        (fileName: 'x_cep_7days.zip', bytes: Uint8List.fromList([2, 3])),
      ];
      expect(findNamedGpsPayload(files, 'cep_7days'), [2, 3]);
    });
