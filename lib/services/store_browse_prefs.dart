...
class StoreBrowsePrefs {
  ...
  String _key(StoreEntryType type, String suffix) =>
      'store_browse_${type.apiValue}_$suffix';

  Future<StoreCatalogQuery> load(StoreEntryType type) async {
    final prefs = await _ensure();
    final sortName =
        prefs.getString(_key(type, 'sort')) ?? StoreSortBy.name.name;
    final dirName =
        prefs.getString(_key(type, 'dir')) ?? StoreSortDirection.ascending.name;
    final priceName =
        prefs.getString(_key(type, 'price')) ?? StorePriceFilter.all.name;
    final category = prefs.getString(_key(type, 'category'));
    final publisher = prefs.getString(_key(type, 'publisher'));
    final starredOnly = prefs.getBool(_key(type, 'starred')) ?? false;

    return StoreCatalogQuery(
      categoryName: (category == null || category.isEmpty) ? null : category,
      publisherName: (publisher == null || publisher.isEmpty)
          ? null
          : publisher,
